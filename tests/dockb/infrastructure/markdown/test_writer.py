...
class TestSerializeBody:
    def test_single_paragraph(self, nlp):
        ...
        assert writer.serialize_body(chapter, nlp) == '<span data-par-id="p1">\nFirst paragraph.\n</span>'
    ...
    def test_sentence_trailing_whitespace_normalizes_to_newline_separator(self, nlp):
        chapter = _chapter(_make_paragraph("First sentence. ", "Second.", p_id="p1"))
        body = writer.serialize_body(chapter, nlp)
    ...
    def test_dirty_chapter_wraps_raw_text_in_single_fresh_span(self, nlp):
        chapter = Chapter(id="c1", title="T")
        chapter.set_text("Dr. Smith arrived. He sat down.")
        body = writer.serialize_body(chapter, nlp)
        spans = re.findall(r'<span data-par-id="([^"]*)">\n([^<]*)\n</span>', body, flags=re.DOTALL)
        assert len(spans) == 1
        assert spans[0][0]
        assert spans[0][1] == "Dr. Smith arrived. He sat down."
    ...
    def test_sentence_less_paragraph_splits_with_nlp(self, nlp):
        ...
        assert re.fullmatch(r'<span data-par-id="[^"]*">\nFoo bar\.\n</span>', body)
