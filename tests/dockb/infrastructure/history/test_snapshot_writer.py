...
def _make_sentence(text: str) -> Sentence:
    """Create a sentence containing the given text."""
    token = Token()
    token.set_text(text)
    sentence = Sentence()
...
def _make_paragraph(*texts: str, p_id: str = "p1") -> Paragraph:
    """Create a paragraph with one sentence per text and a deterministic paragraph ID."""
...
def test_write_body_text_single_paragraph(writer, git_repo):
    ...
    chapter.paragraphs.append(_make_paragraph("First paragraph.", p_id="p1"))
...
def test_write_body_text_multi_paragraph(writer, git_repo):
    ...
    chapter.paragraphs.append(_make_paragraph("Para one.", p_id="p1"))
    chapter.paragraphs.append(_make_paragraph("Para two.", p_id="p2"))
...
def test_write_extras_in_front_matter(git_repo, nlp):
...
def test_write_subsequent_commits_increment(git_repo, nlp):
    writer = SnapshotWriter(base_dir=git_repo, nlp=nlp)
...
def test_write_overwrites_previous_snapshot(git_repo, nlp):
    writer = SnapshotWriter(base_dir=git_repo, nlp=nlp)
...
def test_write_sentence_per_line(writer, git_repo):
    chapter = Chapter(id="c-00000000-0000-0000-0000-000000000012", title="T")
    chapter.paragraphs.append(_make_paragraph("First sentence.", " Second sentence.", p_id="p1"))
...
def test_write_hard_break_preserved(writer, git_repo):
    chapter = Chapter(id="c-00000000-0000-0000-0000-000000000014", title="T")
    chapter.paragraphs.append(_make_paragraph("Line one\\\nline two. Next.", p_id="p1"))
...
def test_write_soft_newline_preserved(writer, git_repo):
    chapter = Chapter(id="c-00000000-0000-0000-0000-000000000015", title="T")
    chapter.paragraphs.append(_make_paragraph("First\nline. Second.", p_id="p1"))
...
def test_write_spans_carry_paragraph_ids(writer, git_repo):
    """The paragraph UUID is written on every sentence span; sentences carry no id."""
    chapter = Chapter(id="c-00000000-0000-0000-0000-000000000016", title="T")
    chapter.paragraphs.append(_make_paragraph("Alpha.", "Beta.", p_id="para-7"))
    chapter.paragraphs.append(_make_paragraph("Gamma.", p_id="para-8"))
...
def test_write_html_escapes_sentence_text(writer, git_repo):
    chapter = Chapter(id="c-00000000-0000-0000-0000-000000000017", title="T")
    chapter.paragraphs.append(_make_paragraph('A <span> tag & "quotes".', p_id="p1"))
...
def test_serialize_is_idempotent(writer):
    chapter = Chapter(id="c-00000000-0000-0000-0000-000000000018", title="T")
    chapter.paragraphs.append(_make_paragraph("First sentence.", " Second sentence.", p_id="p1"))

    assert writer._serialize(chapter) == writer._serialize(chapter)  # pylint: disable=protected-access
